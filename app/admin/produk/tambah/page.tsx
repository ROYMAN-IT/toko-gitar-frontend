...
export default function TambahProduk() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToast({ msg: 'Produk berhasil ditambahkan!', tone: 'ok' });
      setTimeout(() => {
      ...
      setToast({ msg: error.message || 'Gagal menambahkan produk', tone: 'err' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className={`${fraunces.variable} ${inter.variable} ${jetbrainsMono.variable} font-body min-h-screen bg-[#0D0A09] text-white flex antialiased`}>
      ...
      <div className="flex-1 ml-60 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-20 bg-[#0D0A09]/95 backdrop-blur-md border-b border-neutral-800 px-8 py-4 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-1.5 text-[10px] text-neutral-500 select-none mb-0.5">
              ...
              <span className="text-amber-500 font-semibold">Tambah</span>
            </div>
            <h1 className="font-display text-xl font-semibold text-white">Tambah Produk Baru</h1>
          </div>
          <button
        ...
        <main className="flex-1 px-8 py-6">
          <div className="bg-white/[0.03] border border-neutral-800 rounded-2xl p-8 max-w-2xl animate-fade-up">
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {categories.length > 0 ? (
                    ...
                    <>
                      <button
                        type="button"
                        onClick={() => setFormData({ ...formData, category_id: '1' })}
                        className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
                      ...
                      <button
                        type="button"
                        onClick={() => setFormData({ ...formData, category_id: '2' })}
                        className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
                      ...
                      <button
                        type="button"
                        onClick={() => setFormData({ ...formData, category_id: '3' })}
                        className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
                ...
                <select
                  ...
                  className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-xs text-neutral-100 focus:outline-none focus:border-amber-700 transition-colors ${
                ...
                {formErrors.category_id && (
                  <span className="text-[10px] text-red-400 mt-1 flex items-center gap-1">⚠ {formErrors.category_id}</span>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-xs text-neutral-100 focus:outline-none focus:border-amber-700 placeholder:text-neutral-500 transition-colors ${
                    formErrors.name ? 'border-red-700' : 'border-neutral-700'
                  }`}
                />
                {formErrors.name && (
                  <span className="text-[10px] text-red-400 mt-1 flex items-center gap-1">⚠ {formErrors.name}</span>
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-white/5 border border-neutral-700 rounded-xl px-4 py-3 text-xs text-neutral-100 focus:outline-none focus:border-amber-700 placeholder:text-neutral-500 resize-none transition-colors"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {formErrors.price && (
                    <span className="text-[10px] text-red-400 mt-1 flex items-center gap-1">⚠ {formErrors.price}</span>
                  )}
                </div>
                ...
                <div>
                  ...
                  />
                  {formErrors.stock && (
                    <span className="text-[10px] text-red-400 mt-1 flex items-center gap-1">⚠ {formErrors.stock}</span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {formErrors.rating && (
                    <span className="text-[10px] text-red-400 mt-1 flex items-center gap-1">⚠ {formErrors.rating}</span>
                  )}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-full bg-white/5 border border-neutral-700 rounded-xl px-4 py-3 text-xs text-neutral-100 focus:outline-none focus:border-amber-700 placeholder:text-neutral-500 transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="border-t border-neutral-800 pt-4 mt-2">
                <label className="block text-xs font-bold uppercase tracking-wide text-neutral-400 mb-3 flex items-center gap-1.5">
                  <span className="text-amber-500">▸</span> Spesifikasi Produk
                </label>
                <div className="grid grid-cols-2 gap-3 bg-white/[0.02] border border-neutral-800/60 rounded-xl p-3.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white/5 border border-neutral-700 rounded-xl px-3 py-2 text-xs text-neutral-100 focus:outline-none focus:border-amber-700 placeholder:text-neutral-500 transition-colors"
                    />
                  </div>
                  <div>
              ...
              <div>
                ...
                <button
                  ...
                  className="w-full bg-white/5 border-2 border-dashed border-neutral-700 hover:border-amber-700 hover:bg-amber-700/[0.03] rounded-xl px-4 py-6 text-xs text-neutral-400 hover:text-neutral-200 transition-all flex flex-col items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                {imagePreview && (
                  <div className="mt-3 relative inline-block animate-fade-up">
                    <img
                      src={imagePreview}
                      alt="Preview"
                      className="h-40 w-40 object-cover rounded-xl border border-neutral-700 shadow-lg"
                    />
                    <button
                      type="button"
                      onClick={handleRemoveImage}
                      className="absolute -top-2 -right-2 h-6 w-6 rounded-full bg-red-600 hover:bg-red-700 text-white text-xs flex items-center justify-center shadow-lg transition-colors"
                    >
                      ✕
                    </button>
                  </div>
              ...
              {/* PILIHAN (FEATURED) */}
              <div className="bg-white/[0.02] border border-neutral-800/60 rounded-xl px-4 py-3">
              ...
              <div className="flex gap-3 pt-4 border-t border-neutral-800">
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-amber-700 hover:bg-amber-600 disabled:opacity-50 text-white text-xs font-bold uppercase tracking-wide rounded-xl py-3 transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-amber-700/30 active:translate-y-0 disabled:hover:translate-y-0 disabled:hover:shadow-none"
                >
