...
export default function Daftar() {
  ...
  // ✅ TAMBAHKAN STATE TOAST (SAMA SEPERTI CODINGAN KE-2)
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setToast({ msg: 'Password dan konfirmasi password tidak sama.', tone: 'err' });
      return;
    }

    if (!agreeTerms) {
      setToast({ msg: 'Anda harus menyetujui syarat & ketentuan untuk mendaftar.', tone: 'err' });
      return;
    }
    ...
    try {
      ...
      setToast({ msg: 'Akun berhasil dibuat!', tone: 'ok' });
      setTimeout(() => {
        router.push('/auth2');
      }, 1500);

    } catch (error: any) {
      // ❌ HAPUS setError(error.message) - GANTI DENGAN TOAST
      setToast({ msg: error.message, tone: 'err' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <style jsx global>{`
        ...
        .card-rise { animation: cardRise 0.6s cubic-bezier(0.16, 1, 0.3, 1) both; }
        ...
        .field-in-4 { animation: fieldIn 0.5s ease-out 0.34s both; }
        ...
        // ✅ TAMBAHKAN KEYFRAMES fadeUp (SAMA SEPERTI CODINGAN KE-2)
        @keyframes fadeUp { 
        ...
        @media (prefers-reduced-motion: reduce) {
          .drift-slow, .drift-slower, .card-rise, .field-in-1, .field-in-2, .field-in-3, .field-in-4, .shake-err, .animate-fade-up { animation: none !important; }
        }
      `}</style>
      ...
      <div className="card-rise w-full max-w-4xl bg-[#1A1A1A] rounded-3xl overflow-hidden shadow-[0_30px_80px_rgba(0,0,0,0.6)] ring-1 ring-white/[0.05] flex flex-col md:flex-row border border-gray-800 min-h-[590px] relative z-10">
        ...
        <div className="flex-1 bg-[#FFFDF6] text-black p-10 flex flex-col justify-between relative">
          ...
          <div className="my-auto space-y-5">
            <div className="text-center mb-2">
              ...
              <h2 className="font-display text-2xl font-bold tracking-widest uppercase text-gray-800 mt-1">Sign Up</h2>
              <p className="text-[10px] text-gray-400 tracking-wide mt-1">Bergabung dengan komunitas pecinta gitar</p>
            </div>

            {/* ❌ HAPUS BAGIAN ERROR DI SINI - GANTI DENGAN TOAST DI BAWAH */}
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="field-in-1">
                ...
                <div className="relative group/input">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within/input:text-amber-700 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.206" />
                    </svg>
                  </span>
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@example.com"
                    className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-black/[0.02] border border-gray-300 text-sm focus:outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-700/15 transition-all duration-200" />
                </div>
              </div>

              <div className="field-in-2">
                <label className="text-[10px] font-bold tracking-widest uppercase text-gray-400 block mb-1.5">Password :</label>
                <div className="relative group/input">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within/input:text-amber-700 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  ...
                  <input type={showPassword ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••"
                    className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-black/[0.02] border border-gray-300 text-sm focus:outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-700/15 transition-all duration-200" />
              ...
              <div className="field-in-3">
                <label className="text-[10px] font-bold tracking-widest uppercase text-gray-400 block mb-1.5">Konfirmasi Password :</label>
                <input type={showPassword ? 'text' : 'password'} required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••"
                  className="w-full px-4 py-2.5 rounded-xl bg-black/[0.02] border border-gray-300 text-sm focus:outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-700/15 transition-all duration-200" />
              </div>

              <label className="field-in-4 flex items-start gap-2 text-[10px] text-gray-500 tracking-wide cursor-pointer pt-1">
                <input type="checkbox" checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} className="mt-0.5 h-3.5 w-3.5 accent-amber-800 cursor-pointer" />
                <span>Saya menyetujui <span className="underline">syarat & ketentuan</span> serta <span className="underline">kebijakan privasi</span> GitarKu.</span>
              </label>

              <button type="submit" disabled={loading} className="w-full mt-2 py-2.5 bg-amber-800 text-white font-bold rounded-xl text-xs tracking-widest uppercase hover:bg-amber-700 hover:shadow-[0_8px_24px_rgba(180,83,9,0.35)] hover:-translate-y-0.5 transition-all duration-300 shadow-md active:scale-[0.98] flex items-center justify-center gap-2 group cursor-pointer disabled:opacity-50 disabled:hover:translate-y-0">
              ...
              <div className="text-center mt-4">
                <span className="text-[10px] text-gray-400 tracking-wider">SUDAH PUNYA AKUN? </span>
                <button type="button" onClick={() => router.push('/auth2')} className="text-[10px] font-bold hover:underline tracking-wider cursor-pointer text-amber-800">MASUK DI SINI</button>
              </div>
            </form>
          </div>
      ...
      {/* ✅ TAMBAHKAN TOAST DI SINI - SAMA PERSIS DENGAN CODINGAN KE-2 */}
      {toast && (
